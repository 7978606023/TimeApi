...
final private NavigableSet<WorkPeriod> workPeriods
= new TreeSet<>();
// ordered by start time
final private NavigableSet<Event> events = new TreeSet<>();
// ordered by priority
...
List<TaskPart> remainingTaskParts = tasks.stream().map(TaskPart::wholeOf)
.collect(toList());
