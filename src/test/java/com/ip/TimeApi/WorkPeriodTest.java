...
public class WorkPeriodTest {
	...
	public void basicSplitTest() {
		...
		Assert.assertEquals(Optional.of(new WorkPeriod(startTime,midnight)), newPeriod);
		Assert.assertEquals(new WorkPeriod(midnight, endTime),p);
	}
	...
	public void testSplitTwoDayPeriod() {
		...
		LocalDateTime midnight = startLocalDate.plusDays(1).atStartOfDay();
		Assert.assertEquals(Optional.of(new WorkPeriod(startTime, midnight)), split);
		Assert.assertEquals(new WorkPeriod(midnight, endTime), p);
	}
	...
	public void testSplitWithSingleTaskPart() {
		...
		Assert.assertEquals(Optional.of(expectedFirstHalf), newFirstHalf);
		...
		Assert.assertEquals(expectedSecondHalf, p);
	}
	...
	public void testSplitOnEndTime() {
		...
		Assert.assertTrue(p.getEndTime().equals(p.getStartTime()));
	}
