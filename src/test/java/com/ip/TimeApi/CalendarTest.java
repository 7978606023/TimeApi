...
public class CalendarTest {
	...
	public void testAllocateOneTaskSuccess() {
		...
		Assert.assertEquals(1, scheduledPeriods.size());
		List<TaskPart> taskParts = scheduledPeriods.get(0).getTaskParts();
		Assert.assertEquals(1, taskParts.size());
		Assert.assertEquals(t20mins, taskParts.get(0).getOwner());
		Assert.assertEquals(Duration.ofMinutes(20), taskParts.get(0).getDuration());
	}
	...
	public void testAllocateOneTaskFailure() {
		...
		Assert.assertFalse(schedule.isSuccessful());
	}
...
*/
		...
		Assert.assertEquals(scheduleSuccess, periodCanAccommodateTask);
	}
	...
	public void testNoTasksNoPeriods() {
		Schedule schedule = calendar.createSchedule(clock);
		Assert.assertTrue(schedule.getScheduledPeriods().isEmpty());
		Assert.assertTrue(schedule.isSuccessful());
	}
	...
	public void testNowNearEndOfPeriodSucceeds() {
		...
		Assert.assertEquals(1, taskParts.size());
		Assert.assertEquals(Duration.ofMinutes(60), taskParts.get(0).getDuration());
	}
	...
	public void testAllocateTwoPeriodsSuccess() {
		...
		Assert.assertEquals(1, taskParts1.size());
		Assert.assertEquals(t30mins, taskParts1.get(0).getOwner());
		Assert.assertEquals(Duration.ofMinutes(20), taskParts1.get(0).getDuration());
		List<TaskPart> taskParts2 = schedule.get(1).getTaskParts();
		Assert.assertEquals(1, taskParts2.size());
		Assert.assertEquals(t30mins, taskParts2.get(0).getOwner());
		Assert.assertEquals(Duration.ofMinutes(10), taskParts2.get(0).getDuration());
	}
	...
	public void testAllocateTwoTasksSuccess() {
		...
		List<TaskPart> taskParts = schedule.get(0).getTaskParts();
		Assert.assertEquals(2, taskParts.size());
		Assert.assertEquals(t20mins, taskParts.get(0).getOwner());
		Assert.assertEquals(Duration.ofMinutes(20), taskParts.get(0).getDuration());
		Assert.assertEquals(t30mins, taskParts.get(1).getOwner());
		Assert.assertEquals(Duration.ofMinutes(30), taskParts.get(1).getDuration());
	}
